...
1. 写出一个 bash 脚本，可以使它自动读取[文件夹 linux_homework.zip](../../files/linux_homework.zip)的内容，将文件的名字输出到 filenames.txt, 将文件夹的名字输出到 dirname.txt 。 
要求：下载linux_homework.zip并解压。将 bash 脚本， filename.txt dirname.txt 写到同一个文件中上交。格式建议： word, pdf, txt.
