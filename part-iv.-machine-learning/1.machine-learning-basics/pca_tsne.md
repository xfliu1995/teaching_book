...
其中，data行为样本，列为特征；sampleclass为样本对应的标签；method\_PCA为是否使用PCA，当为True时，使用PCA方法，False时使用t-SNE方法；method\_StandardScaler是指是否对数据进行归一化处理；size\_dot是画图时点的大小camshaft。
