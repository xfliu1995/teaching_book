...
\(1b\) 进入到工作目录 （以下步骤均在 `/home/test/blast/` 下进行）:
...
1. 你可以利用 `more`, `less`等命令或者利用vi等文本编辑工具查看结果文件。
...
> **注：** blast的网站会提供多个mouse的databases，可以任选1个，也可以选多个比较和讨论一下输出结果。

* **挑战题:** 

我们常见的PAM250有如下图所示的两种（一种对称、一种不对称），请阅读一下[Symmetry\_of\_the\_PAM\_matrices](https://en.wikipedia.org/wiki/Point_accepted_mutation#Symmetry_of_the_PAM_matrices), 再利用Google/wikipedia等工具查阅更多资料，然后总结和解释一下这两种（对称和不对称）PAM250不一样的原因及其在应用上的不同。

> PAM matrices are also used as a scoring matrix when comparing DNA sequences or protein sequences to judge the quality of the alignment. This form of scoring system is utilized by a wide range of alignment software including [BLAST](https://en.wikipedia.org/wiki/BLAST).[\[15\]](https://en.wikipedia.org/wiki/Point_accepted_mutation#cite_note-15)

[不对称的PAM250](http://www.deduveinstitute.be/~opperd/private/pam250.html)：![Figure 83. Atlas of Protein Sequence and Structure, Suppl 3, 1978, M.O. Dayhoff, ed. National Biomedical Research Foundation, 1979](../.gitbook/assets/pam250-2.png) 

[对称的PAM250](https://en.wikipedia.org/wiki/Point_accepted_mutation#An_example_-_PAM250)： ![](../.gitbook/assets/pam250-1.png)
