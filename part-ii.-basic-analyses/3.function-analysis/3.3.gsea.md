...
* 程序下载与安装（Docker 中均已安装好, 参考[文件获取方式](README.md#files)）：
...
* 准备数据：至 [GEO](https://www.ncbi.nlm.nih.gov/geo/browse/?view=series&display=20) 查找感兴趣的数据集（本例中我们使用 [GSE19161](https://www.ncbi.nlm.nih.gov/geo/query/acc.cgi?acc=GSE19161)），下载 `series_matrix.txt.gz` 文件和`family.soft.gz` 文件（参考[文件获取方式](README.md#files)）。
