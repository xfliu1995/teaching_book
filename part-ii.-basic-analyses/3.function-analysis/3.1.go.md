...
参考 [文件获取方式](./#files)下载 GO\_gene.txt 文件, 其内容如下所示，包含了很多基因的ID：
...
1. 请选取上面基因中的一个子集（建议30-50个之间），并重新进行GO enrichment的分析，看看不同子集会有什么样的结果（有些时候可能enrich不出合适的GO term）；然后从一次有效的结果中举一个 GO term 的例子，列出 expected, Fold Enrichment, raw P value等，并给出相应的统计计算公式。
